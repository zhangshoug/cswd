...
def fetch_qq_industry_categories():
    """腾讯行业分类"""
    id_p = re.compile('a-l-bd01\d{4}')
    href = re.compile('id=bd01\d{4}')
...
def fetch_region_categories():
    """地域分类"""
    id_p = re.compile('a-l-bd03\d{4}')
    href = re.compile('id=bd03\d{4}')
    df = _fetch_categories(id_p, href)
    return df

def fetch_csrc_industry_categories():
    """证监会行业分类"""
    id_p = re.compile('a-l-bd04[A-Z]{2}\d{2}')
    href = re.compile('bd04[A-Z]{2}\d{2}')
    df = _fetch_categories(id_p, href)
    return df
...
def _fetch_one_item_stocks(item_id, item_name):
    ...
    df = pd.DataFrame({'item_id':item_id, 'item_name':item_name, 'code':codes})
    return df
...
def _fetch_item_stocks(item_data):
    ...
    with click.progressbar(item_data.iterrows(),
                           ...
                           label="Fetch item stock list",) as bar:
        for x in bar:
            item_id = x[1][0][-6:]
            item_name = x[1][1]
...
def get_recent_trading_stocks():
    ...
    return df.query('成交量>0').index.str.slice(2,8).values
