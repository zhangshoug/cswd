...
_BASE_PATTERN = re.compile('^(?:A股)?(?P<base>\d{2,})\D')
_INCREASE_PATTERN = re.compile('(?<=转增)(\d+(?:\.\d+)?)股')
_GIVE_PATTERN = re.compile('(?<=送)(\d+(?:\.\d+)?)\D')
_DIVIDEND_PATTERN = re.compile('(?<=派)(\d+(?:\.\d+)?)元')
...
def fetch_index_info():
    ...
    f = lambda x:pd.to_datetime(x, format='%Y-%m-%d', errors='coerce')
...
def fetch_announcement_summary():
    ...
    output = pd.DataFrame({
        'code':data['secCode'].values,
        'name':data['secName'].values,
        'time':data['announcementTime'].apply(pd.Timestamp, unit = 'ms'),
        'title':data['announcementTitle'].values,
        'type':data['announcementType'].values,
        'type_name':data['announcementTypeName'].values,
    })
    return output
...
def fetch_industry(date_str, department):
    ...
    try:
        ...
        raise ValueError("请检查日期'{}'是否为交易日期".format(date_str))
